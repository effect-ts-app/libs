...
export const map = <TIn, TOut, T extends D.Dictionary<TIn>>(f: (a: TIn) => TOut) =>
  (fa: T & D.Dictionary<TIn>) => map_<TIn, TOut, T>(fa, f)

export const mapWithIndex = <TIn, TOut, T extends D.Dictionary<TIn>>(f: (i: keyof T, a: TIn) => TOut) =>
  (fa: T & D.Dictionary<TIn>) => mapWithIndex_<TIn, TOut, T>(fa, f)
