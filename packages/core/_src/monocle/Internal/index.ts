 ...
 */
export const isoComposeIso = <A, B>(ab: Iso<A, B>) =>
...
export const isoAsTraversal = <S, A>(sa: Iso<S, A>): Traversal<S, A> =>
  new Traversal({
    modifyF: <F extends P.URIS, C = P.Auto>(F: P.Applicative<F, C>) =>
...
export const lensComposeLens = <A, B>(ab: Lens<A, B>) =>
...
export const lensComposePrism = <A, B>(ab: Prism<A, B>) =>
  <S>(sa: Lens<S, A>): Optional<S, B> => optionalComposeOptional(prismAsOptional(ab))(lensAsOptional(sa))

export const lensId = <S>(): Lens<S, S> => new Lens({ get: identity, set: constant })

export const lensProp = <A, P extends keyof A>(prop: P) =>
...
export const lensProps = <A, P extends keyof A>(...props: [P, P, ...Array<P>]) =>
...
export const lensComponent = <A extends ReadonlyArray<unknown>, P extends keyof A>(prop: P) =>
  <S>(lens: Lens<S, A>): Lens<S, A[P]> =>
    new Lens({
      get: s => lens.get(s)[prop],
      set: ap =>
...
export const prismModifyOption = <A>(f: (a: A) => A) =>
  <S>(sa: Prism<S, A>) =>
    (s: S): O.Option<S> =>
...
export const prismModify = <A>(f: (a: A) => A) =>
...
export const prismComposeLens = <A, B>(ab: Lens<A, B>) =>
  <S>(sa: Prism<S, A>): Optional<S, B> => optionalComposeOptional(lensAsOptional(ab))(prismAsOptional(sa))
...
export const optionalModifyOption = <A>(f: (a: A) => A) =>
...
export const optionalModify = <A>(f: (a: A) => A) =>
  <S>(optional: Optional<S, A>): ((s: S) => S) => {
    const g = optionalModifyOption(f)(optional)
    return s => O.getOrElse_(g(s), () => s)
  }

export const optionalComposeOptional = <A, B>(ab: Optional<A, B>) =>
...
export const find: <A>(predicate: Predicate<A>) => Optional<ReadonlyArray<A>, A> = findMutable as any
  ...
  return <S>(sa: Traversal<S, A>) =>
    new Traversal({
      modifyF: <F extends P.URIS, C = P.Auto>(F: P.Applicative<F, C>) =>
        ...
        ): ((s: S) => P.Kind<F, C, FK, FQ, FW, FX, FI, FS, FR, FE, S>) => sa.modifyF(F)(ab.modifyF(F)(f))
    })
}
