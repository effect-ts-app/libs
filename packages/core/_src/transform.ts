...
type MaybeOf<A> = Union.Exclude<
  A extends O.Some<infer X> ? X | null : A,
  O.None
>
// eslint-disable-next-line @typescript-eslint/no-explicit-any

export type TransformRoot<O> = O extends O.Maybe<any> ? Transform<MaybeOf<O>>
  : Transform<O>
export type Transform<O> = O extends Misc.BuiltIn | Misc.Primitive ? O
  : {
    [K in keyof O]: MaybeOf<O[K]> extends infer X ? X extends (infer Y)[] ? MaybeOf<Transform<Y>>[]
    : X extends NA.NonEmptyArray<infer Y> ? NA.NonEmptyArray<MaybeOf<Transform<Y>>>
    : X extends SET.Set<infer Y> ? SET.Set<MaybeOf<Transform<Y>>>
    : X extends readonly (infer Y)[] ? readonly MaybeOf<Transform<Y>>[]
    : Transform<X>
      : never
  }

export const encodeMaybesAsNullable = <T>(root: T): TransformRoot<T> => encodeMaybesAsNullable_(root, new Map())
