...
export const LiveServiceBusClient = (url: string) => makeClient(url).scoped(Client)
...
export function LiveSender(queueName: string) {
  return makeSender(queueName).scoped(Sender)
}
...
export function LiveReceiver(queueName: string) {
  return makeReceiver(queueName).scoped(Receiver)
}
...
export function Subscription<RMsg, RErr>(hndlr: MessageHandlers<RMsg, RErr>) {
  return subscribe(hndlr).scoped(SubscribeTag)
}
