...
export const provideChildLogger = (meta: LOG.Meta) =>
  Effect.environmentWithEffect(env => makeChild(meta).map(child => env.add(WinstonInstance, child)))
...
export const makeWinstonLogger = Effect.gen(function*($) {
  ...
  return {
    debug: (message, meta) =>
      Effect.serviceWithEffect(WinstonInstance, _ => Effect.sync(() => _.logger.log("debug", message, meta))) as any,
    http: (message, meta) =>
      Effect.serviceWithEffect(WinstonInstance, _ => Effect.sync(() => _.logger.log("http", message, meta))) as any,
    silly: (message, meta) =>
      Effect.serviceWithEffect(WinstonInstance, _ => Effect.sync(() => _.logger.log("silly", message, meta))) as any,
    error: (message, meta) =>
      Effect.serviceWithEffect(WinstonInstance, _ => Effect.sync(() => _.logger.log("error", message, meta))) as any,
    info: (message, meta) =>
      Effect.serviceWithEffect(WinstonInstance, _ => Effect.sync(() => _.logger.log("info", message, meta))) as any,
    verbose: (message, meta) =>
      Effect.serviceWithEffect(
        WinstonInstance,
        _ => Effect.sync(() => _.logger.log("verbose", message, meta))
      ) as any,
    warn: (message, meta) =>
      Effect.serviceWithEffect(WinstonInstance, _ => Effect.sync(() => _.logger.log("warn", message, meta))) as any
  } as LOG.Logger
})

export const WinstonLogger = LiveWinstonInstance > Layer.fromEffect(LOG.Logger)(makeWinstonLogger)
